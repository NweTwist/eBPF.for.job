...
int main(int argc, char **argv)
{
    ...
    bpf_object__for_each_program(prog, obj) {
        ...
        if (sec && strcmp(sec, "cgroup_skb/egress") == 0) {
    ...
    err = bpf_prog_attach(prog_fd, cg_fd, BPF_CGROUP_INET_EGRESS, 0);
    if (err) {
    ...
    printf("Attached skb egress prog fd=%d to cgroup '%s' (fd=%d)\n", prog_fd, cg_path, cg_fd);
    ...
    bpf_prog_detach2(prog_fd, cg_fd, BPF_CGROUP_INET_EGRESS);
    bpf_object__close(obj);
    close(cg_fd);
